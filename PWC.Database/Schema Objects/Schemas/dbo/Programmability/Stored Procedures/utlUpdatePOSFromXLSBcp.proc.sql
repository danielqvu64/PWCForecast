CREATE PROCEDURE [dbo].[utlUpdatePOSFromXLSBcp]
AS
SET NOCOUNT ON
...
SELECT @errorNumber = 0
	...
	, @errorState = 0
	, @errorLine = 0
	, @errorProcedure = 'utlUpdatePOSFromXLSBcp'
	, @rowBackedUp = 0
	, @duplicateRowDeleted = 0
	, @rowInserted = 0

BEGIN TRY
	SELECT	'001' company_code, '1278' customer_number, '01-' + RIGHT(p.F3, 5) + '-00' item_number, w.week_end_date, SUM(CAST(p.F5 as int)) quantity
	INTO	#newPOS
	FROM	POSXlsBcp p
	JOIN	WeekCalendar w
		ON	CAST(RIGHT(p.F4, 8) AS datetime) BETWEEN w.week_start_date AND w.week_end_date
	WHERE	p.F3 IS NOT NULL
	AND	p.F4 IS NOT NULL
	AND	p.F5 IS NOT NULL
	GROUP BY '01-' + RIGHT(p.F3, 5) + '-00', w.week_end_date
	ORDER BY 1, 2, 3, 4
